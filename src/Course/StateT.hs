...
instance Monad k => Applicative (StateT s k) where
  ...
  (<*>) ::
    ...
    -> StateT s k a
    -> StateT s k b
  (<*>) sab sa = StateT (\s -> (\(fab, s') -> (\(a, s'') -> (fab a, s'')) <$> runStateT sa s') =<< runStateT sab s)
    --error "todo: Course.StateT (<*>)#instance (StateT s k)"
...
instance Monad k => Monad (StateT s k) where
  (=<<) ::
    ...
    -> StateT s k a
    -> StateT s k b
  (=<<) fasb sa = StateT (\s -> (\(a, s') -> runStateT (fasb a) s') =<< runStateT sa s)
    -- error "todo: Course.StateT (=<<)#instance (StateT s k)"
...
exec' ::
  ...
  -> s
  -> s
-- exec' se s = snd $ runState' se s
exec' se = runExactlyOne . execT se
...
distinct' xs = eval' (filtering
                       (\a -> (\s -> S.notMember a s <$ putT (S.insert a s)) =<< getT)
           xs) S.empty
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF xs = filtering (\a -> if a >= 100 then Empty else Full True) (distinct' xs)
...
instance Monad k => Applicative (OptionalT k) where
  ...
  (<*>) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  -- (<*>) f a = let m = (\p -> onFull (\a -> ) p) <$> runOptionalT a in _
  -- (<*>) f a = let p = lift2 (\d c -> c <*> d) (runOptionalT a) (runOptionalT f) in OptionalT p
...
--     where f = (\a -> StateT (\s -> OptionalT (
--                 (
--                   Logger Nil (
--                     if a > 100 then Empty else Full (S.notMember a s, S.insert a s)
--                   )
--                 ) <* logElement a
...
logElement :: (Show a, Integral a) => a -> Logger Chars ()
logElement a = 
    if a > 100 
      then log1 ("aborting > 100: " ++ show' a) ()
      else if even a then log1 ("even number: " ++ show' a) ()
      else pure ()
